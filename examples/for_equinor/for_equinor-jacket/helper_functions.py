...
def write_optimized_yaml(forig, yaml_opt, ftmp):
    ...
    for line in orig:
        if not flag_tow and line.find('tower:') >= 0:
            flag_tow = True
        if not flag_diam and flag_tow and line.find('outer_diameter:') >= 0:
            flag_diam = True
        if not flag_thick and flag_tow and line.find('thickness:') >= 0:
            flag_thick = True
        ...
        if flag_tow and flag_diam and line.find('values:') >= 0:
            ...
            flag_key = False
            for key in ['r_foot','r_head','leg_thickness','leg_diameter','bay_spacing','brace_diameters','brace_thicknesses']:
                if line.find(f'{key}:') >= 0:
                    flag_key = True
                    val = yaml_opt['components']['jacket'][key]
                    tok = line.split(':')
                    fw.write(f'{tok[0]}: {val}\n')

            if not flag_key: fw.write(line)
